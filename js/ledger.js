...
async function loadMemberLedger() {
    ...
    const gsList = gs.filter(g => m.groupIds && m.groupIds.includes(g.id));
    ...
    function buildSection(grp, enr, slotPays, slotNum, totalSlots, allDueDates, sectionId) {
        ...
        // --- CRITICAL FIX: SEARCHING FOR THE EXACT FIREBASE FIELD ---
        // This checks all common names for your "Fixed Monthly Amount" field
        ...
        // Calculate paid months using the found chitAmount
        ...
        const mergedRows = allDueDates.map((dueDate, slotIndex) => {
            ...
            if (monthPayments.length === 0) {
                ...
                return `<tr>
                    ...
                    <td colspan="3" style="text-align:center;color:var(--text-dim);">—</td>
            ...
            return `
                <tr style="cursor:${hasMultiple ? 'pointer' : 'default'};" onclick="${hasMultiple ? `togglePaymentDetails(this,'${detailClass}')` : ''}">
                    ...
                    <td style="font-size:0.7rem;color:var(--text-dim);">${mainPay.paidBy || '—'}</td>
                ...
                ${hasMultiple ? monthPayments.map((p, idx) => `
                    <tr class="${detailClass}" style="display:none; background:rgba(99,102,241,0.03); border-left:3px solid #6366f1;">
                        <td style="text-align:right; color:#818cf8; font-size:0.6rem; font-weight:800;">↳${idx + 1}</td>
                        ...
                        <td style="font-weight:700; color:#fbbf24;">${fmtAmt(p.paid)}</td>
                        <td style="color:#f59e0b;">${fmtAmt(p.balance)}</td>
                        <td colspan="2" style="font-size:0.65rem; color:var(--text-dim);">${p.paidBy || '—'}</td>
        ...
        <div class="member-card">
            <div style="padding:15px; background:rgba(28,37,59,0.5); border-radius:12px 12px 0 0;">
                <h5 style="color:var(--gold); font-weight:900;">Group: ${grp.name}</h5>
